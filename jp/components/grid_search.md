...
|制限|説明|
|--- |--- |
|テンプレート付きセルの検索|検索機能がセル テンプレートを div 要素とスパンで置き換えてセル値の検索テキストを強調表示します。そのため、テンプレートに関連付けした複数の要素やイベントを含む複雑なテンプレートがある場合に予期しない動作を発生する場合があります。列にそのようなテンプレートがある場合、列フォーマッターや列の [`searchable`]({environment:angularApiUrl}/classes/igxcolumncomponent.html#searchable) プロパティを false に設定するなど、他の方法を使用してください。|
